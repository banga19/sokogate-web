        ...
        <b-row class="login-pad">
          ...
          <b-col cols="10" md="4" class="centre">
             <b-card class="login-card" :title="$t('login.title')">
               <div class="google-signin-wrapper" v-show="!showQRCode">
                 <button
                   ref="googleBtnRef"
...
import { Login, LoginQRCode, GetLoginCheckStatus, GoogleSSO } from '@/utils/api'
  ...
  methods: {
    ...
    async googleLogin() {
      ...
      try {
        ...
        const res = await GoogleSSO({ idToken })
        this.loginSuccess(res)
      } catch (err) {
        console.log('google-login-err:', err.message || err)
        this.$bvToast.toast('Google login failed. Please try again.', {
