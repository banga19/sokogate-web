...
export default {
  ...
  methods: {
    loadMapScript() {
      ...
      // 此处需要注意: requestak时, 一定要应用类别一定要选浏览器端, 不能选server, 不然地图会报ak无效
      script.onload = () => {
    ...
    //   console.log(window.BMap, 'window');
      let Bmap = window.BMap; // 注意要带window, 不然会报错 (注意官方api,会有改动, 之前是Bmap,后面3.0版本改为了BMap,最好查文档或者print一下window) 
      var map = new Bmap.Map("localtion-box"); // allmap必须和dom上的id一直
      // var cityCtrl = new Bmap.CityListControl(); // Add城市list控件
    ...
    address: 广东省广州市越秀区环市东路372号正佳东方国际广场5001房
    </p>
    <p style='margin:0;line-height:1.5;font-size:13px;text-indent:2em'>
    电话: +86 020 33972585
    </p></div>`;
