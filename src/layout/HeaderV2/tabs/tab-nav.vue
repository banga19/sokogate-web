...
function addResizeListener(element, handler) {
  if (window.ResizeObserver) {
    const resizeObserver = new ResizeObserver(handler);
    resizeObserver.observe(element);
    element.__vueResizeObserver = resizeObserver;
    return resizeObserver;
  }
  element && element.addEventListener && element.addEventListener('resize', handler);
}

function removeResizeListener(element, handler) {
  if (element && element.__vueResizeObserver) {
    element.__vueResizeObserver.disconnect();
    delete element.__vueResizeObserver;
  }
  element && element.removeEventListener && element.removeEventListener('resize', handler);
}
